...
bool Paddle::collides (SDL_Rect objectRect, Vector2D &collVector) {
	...
	bool boundariesOK;
	int xCenterObject = objectRect.x + (objectRect.w / 2);
	int yBottomObject = objectRect.y + objectRect.h;

	// checks that the object's collision points are within the paddle's boundaries
	boundariesOK = yBottomObject > position.getY () && yBottomObject < (position.getY () + height);
	boundariesOK &= xCenterObject > position.getX () && xCenterObject < (position.getX () + width);

	if (boundariesOK) {
		int mid = position.getX () + (width / 2);
		double n = cos (MAX_ANGLE * RADIAN_CONVERSION_FACTOR);  

		collVector.setX ((n * double(xCenterObject - mid)) / (position.getX () - double(mid)));
...
void Paddle::update () {
	mapWidth = playState->getMapWidth ();
